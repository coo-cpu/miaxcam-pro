...
- The synthesized move operations perform member-wise initialization/assignment of the data members, in their declaration order, from an *xvalue* ${}^{\textcolor{red}{1}}$ argument.
  - i.e. `X(X&& o) : mem1(std::move(o.mem1)), mem2(std::move(o.mem2)), ... {}`. (Later we will talk about `std::move`.)
